...
class SubmissionAv2:
    ...
    def format_data(
        ...
        # Changed from .double to .to(torch.float32)
        origin = data["origin"].view(batch, 1, 1, 2).to(torch.float32)
        theta = data["theta"].to(torch.float32)
        ...
        with torch.no_grad():
            global_trajectory = (
                # Changed from .double to .to(torch.float32)
                torch.matmul(trajectory[..., :2].to(torch.float32), rotate_mat.unsqueeze(1))
                + origin
            )
            if not normalized_probability:
                # Changed from .double to .to(torch.float32)
                probability = torch.softmax(probability.to(torch.float32), dim=-1)
